...
const vnpayPaymentItems = asyncHandler(async (req, res) => {
    ...
    var orderId = formatDate2(date, 'HHmmss');
    ...
    vnp_Params['vnp_ExpireDate'] = formatDate(date, 'yyyymmddHHmmss')
...
const momoPaymentCheck = asyncHandler(async (req, res) => {
    var momo_Params = req.query;
    var secureHash = momo_Params['momo_SecureHash'];

    delete momo_Params['momo_SecureHash'];
    delete momo_Params['momo_SecureHashType'];

    momo_Params = sortObject(momo_Params);
    var secretKey = process.env.accessKey
    var signData = querystring.stringify(momo_Params, { encode: false });
    var hmac = crypto.createHmac("sha256", secretKey);
    var signed = hmac.update(new Buffer(signData, 'utf-8')).digest("hex");


    if (secureHash === signed) {
        var orderId = momo_Params['momo_TxnRef'];
        var rspCode = momo_Params['momo_ResponseCode'];
        //Kiem tra du lieu co hop le khong, cap nhat trang thai don hang va gui ket qua cho momoAY theo dinh dang duoi

        const order = await Order.findById(momo_Params['OrderInfo'])
        if (order) {
            order.isPaid = true
            order.paidAt = Date.now()
